...
var SwipeView = (function (window, document) {
	...
	var dummyStyle = document.createElement('div').style,
		...
		startEvent = hasTouch ? 'touchstart' : '',
		moveEvent = hasTouch ? 'touchmove' : '',
		endEvent = hasTouch ? 'touchend' : '',
		cancelEvent = hasTouch ? 'touchcancel' : '',
		...
		SwipeView = function (el, options) {
			...
			for (i=-1; i<2; i++) {
				...
				div.style.cssText = cssVendor + 'transform:translateZ(0);position:absolute;top:0;height:100%;width:100%;left:0;top:' + i*100 + '%';
...
/*			if (!hasTouch) {
		...
		refreshSize: function () {
			...
			this.maxX = -this.options.numberOfPages * this.pageHeight + this.wrapperHeight;
		...
		next: function () {
			if (!this.options.loop && this.y == this.maxX) return;
		...
		*/
		__pos: function (x) {
			this.y = x;
			this.slider.style[transform] = 'translate(0, ' + x + 'px)' + translateZ;
		},
...
/*			var matrix = getComputedStyle(this.slider, null).webkitTransform.replace(/[^0-9-.,]/g, '').split(',');
		...
		__move: function (e) {
			...
			this.pointX = point.pageX;
			this.pointY = point.pageY;
			this.directionX = deltaY > 0 ? 1 : deltaY < 0 ? -1 : 0;
			this.stepsX += Math.abs(deltaX);
			this.stepsY += Math.abs(deltaY);

			// We take a 10px buffer to figure out the direction of the swipe
			if (this.stepsY < 10 && this.stepsX < 10) {
//				e.preventDefault();
				return;
			}
...
/*			if (newX > 0 || newX < this.maxX) {
		...
		__end: function (e) {
			...
			if (!this.options.loop && (this.y > 0 || this.y < this.maxX)) {
				dist = 0;
				this.__event('movein');
			}
		...
		__checkPosition: function () {
			...
			newX = -this.page * this.pageHeight;

			this.isMoving = true;
			setTimeout(function() {
				custom_this.isMoving = false;
			}, Math.floor(1000 * Math.abs(this.y - newX) / this.pageHeight))
			this.slider.style[transitionDuration] = Math.floor(1000 * Math.abs(this.y - newX) / this.pageHeight) + 'ms';
